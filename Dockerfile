...
ENV MYSQL_USER_NAME cook
ENV MYSQL_PASSWORD Cook2022
ENV DATABASE_NAME cook
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/cook-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/cook-0.0.1.jar", "--spring.profiles.active=test"]